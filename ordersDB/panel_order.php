...
if ($_POST){
    ...
    //If it exists, delete it
        ...
        $stmt = mysqli_stmt_init($conn);
// 2+16
        $query = "INSERT INTO order_entry (email, reid, orderTag, shadeID, quantity, width, length, measure, `group`, series, fabric, hem, hemColor, hemCaps, hemStitched, controlPosition, controlSystem, controlColorPower, controlController, controlClutchMotor, controlclutchCover, valance, valanceReturn, rollType, shade, price, panel, `open`, channel, PO, stationary) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?);";
        if(!mysqli_stmt_prepare($stmt, $query)){
            ...
            //ssssiddsssssssssssssssssssssisssssssssssdsss
            mysqli_stmt_bind_param($stmt, "ssssiddsssssssssssssssdssdsssss",$email, $reid,$orderTag,$shadeID,$quantity,$width,$length,$measure,$group,$series,$fabric,$hem,$hemColor,$hemCaps, $stitched ,$controlPosition,$controlSystem,$controlColorPower,$controlController,$controlClutchMotor,$controlClutchCover,$valance,$valanceReturn,$rollType, $shade, $price, $panel, $open, $channel, $purchaseOrder, $stationary);
            mysqli_stmt_execute($stmt);
            echo ('Success Rei');
...
//else If No Post
    ...
    header("Location: ../r2.html?result=FailedPost");
    exit();
}
