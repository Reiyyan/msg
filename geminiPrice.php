...
//Variables
        ...
        $back_fabric_name = $_POST['back_fabric_name'];
        $back_price_group = $_POST['back_price_group'];
        ...
        $back_drive = $_POST['back_motor_clutch'];
        $back_control_system = $_POST['back_control_system'];
...
//Fabric Price
    if(!($fabric_name == "")){
        ...
        if($shade == 'roller shade' || $shade == 'vision shade' || $shade = 'gemini dual shade'){
...
//Back Fabric Price
    if(!($back_fabric_name == "")){
        ...
        if($shade == 'gemini dual shade'){
            ...
            $fQuery = "SELECT `$width` FROM `_roller_pg_$back_price_group` WHERE length = $length;";
            //echo $fQuery;
            //echo '<br>';
            $fResult = $conn->query($fQuery);
            if ($fResult->num_rows == 1) {
...
//Back Drive Price
    ...
    if($back_control_system == 'Motor'){
        if(!is_null($back_drive)){
        $dQuery = "SELECT price FROM _drive_price WHERE drive = '$back_drive';";
        //echo $dQuery;
        //echo '<br>';
        $dResult = $conn->query($dQuery);
            if ($dResult->num_rows == 1) {
...
/*---------------------------------------------------------------------------------------------------------------------------------------*/
$total = $fabric_price +$back_fabric_price + $drive_price +$back_drive_price + $valance_price + $trim_price + $pull_price + $side_price + $channel_price + $lam_price;

$json = array('discount' => $discount, 'total' => $total, 'lam' => $lam_price);
